...
0. arguments 对象不是数组，而是一个类数组对象。<br/>
   rest 参数是一个真正的数组。

1. rest 参数只能是函数最后一个参数，否则会报错。
...
1. 获取一个具名函数的 name 属性，，则 ES5 和 ES6 都返回这个函数原本的名字。
...
2. Function 构造函数返回的函数实例，name 属性的值为 anonymous

    ```javascript
    new Function().name; // "anonymous"
    ```

3. bind 返回的函数，name 属性值会加上 bound 前缀。
...
1. 函数体内没有 arguments 对象。如果要访问参数，可以用 rest 参数。
2. 不可以使用 new 命令，因此不能用作构造函数，会出错。
