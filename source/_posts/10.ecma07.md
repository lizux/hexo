...
> 注：NodeList 对象比 HTMLCollection 对象 多提供了一个 forEach() 方法（仅现代浏览器，IE 不支持）。
    属于 DOM Core 规范。
...
* insertBefore()
    它接受两个参数，第一个参数是所要插入的目标节点，第二个参数是目标节点的一个子节点，新的节点将插在这个子节点的前面。如果第二个参数为 null，则新节点将插在目标节点的最后位置，即变成最后一个子节点。
